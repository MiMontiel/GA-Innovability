...
//Branch 1: Crossover by Cluster. Parents exchange a randomly selected cluster. 
...
void evolve(int xs) { //
 ...
 for (int i = 0; i < reproduction_control; i++) { // for as many times as population size... // in the future this is the point where the size of population could vary depending on average fitness or other factors 
    ...
    if(dice <= crossover_rate){//Crossover occurs
      ...
      output.print(i + ":" + "\t" + aCluster + "\t" + cs);//Add data to .csv file. i = offspring id/ aCluster = exchanged cluster / cs = size of exchanged cluster
      output.print("\n");
      ...
      ArrayList<Integer> offsp = new ArrayList<Integer>(); //Create ArrayList
      float dice2 = random(-1,1);
      if(dice2 <=0){
        offsp.addAll(parent1); // an identical copy of parent 1 
      }else if(dice2 > 0){
        offsp.addAll(parent2); // an identical copy of parent 2
      }
      offspring_pop.add(offsp);
      output.print(i + ":" );//Add data to .csv file. i = offspring id
      output.print("\n");
    }
    }//Closes reproduction option 1
    
    //Reproduction option 2: Each pair of parents produces TWO offspring
    if(which_scheme == 2){
      float dice = random(0,100);
      if(dice <= 80){//Crossover occurs
        int aCluster = (int)random(0, numOfClusters); // select a random cluster to determine the crossover point
        int cs = clusterSizes[aCluster], cp = clusterAnchors[aCluster]; // size and anchor of each cluster
        // could create two offsprings and build their phenotype of BOTH to select the fittest, but let's go random...
        track_c++; // Update the number of crossover episodes that have taken place 
        ...
        ArrayList<Integer> offsp1 = new ArrayList<Integer>(); //Create ArrayList
        ArrayList<Integer> offsp2 = new ArrayList<Integer>(); //Create ArrayList
      
        offsp1.addAll(parent1); // an identical copy of parent 1, except...
        for (int a = 0; a < cs; a++) offsp1.set(a+cp, parent2.get(a+cp)); // for the selected cluster, from parent 2
      
        offsp2.addAll(parent2); // the opposite...
        for (int a = 0; a < cs; a++) offsp2.set(a+cp, parent1.get(a+cp));
      
        offspring_pop.add(offsp1);
        offspring_pop.add(offsp2);
      }else if(dice > 80){//Crossover doesn't occur
        ArrayList<Integer> offsp1 = new ArrayList<Integer>(); //Create ArrayList
        ArrayList<Integer> offsp2 = new ArrayList<Integer>(); //Create ArrayList
        offsp1.addAll(parent1); // an identical copy of parent 1
        offsp2.addAll(parent2); // an identical copy of parent 1
        offspring_pop.add(offsp1);
        offspring_pop.add(offsp2);
