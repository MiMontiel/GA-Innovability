...
//Branch 3: Crossover by Section. Parents exchange a section of size "exchanged_section"
...
//Drawing control. If set to "true" phenotyes are drawn. If set to "false" phenotypes are not drawn
...
int exchanged_section = int(g_length * 0.1);//Length of the section genotypes exchange during crossover 
...
void evolve(int xs) {
  ...
  // Fourth, Crossover (parents exchange sections)
  ...
  for (int i = 0; i < reproduction_control; i++) { // for as many times as reproduction_control... // in the future this is the point where the size of population could vary depending on average fitness or other factors 
    ...
    if(which_scheme == 1){
      ...
      if(dice <= crossover_rate){//Crossover occurs
        int cp = int(random(0,g_length - exchanged_section));//Select a location within parents' genotypes
        track_c++;//Update the numbers of crossover episodes that have taken place
        
        ArrayList<Integer> section_p1 = new ArrayList<Integer>(parent1.subList(cp,cp+exchanged_section)); // A subList that stores a section of parent 1
        ArrayList<Integer> section_p2 = new ArrayList<Integer>(parent2.subList(cp,cp+exchanged_section)); // A subList that stores a section of parent 2
        
        e_genes = e_genes + section_p1.size();//Update the total number of genes that have been exchanged  
        
        output.print(i + ":" + "\t" + cp + "\t" + section_p1.size());//Add data to .csv file. i = offspring id/ cp = crossover point / section_p1.size() = length of the exchanged section
        output.print("\n");
        
        int crossover_counter = cp; // Parents exchange the sections specified above
        for(int k = 0; k < exchanged_section; k++){
          parent1.set(crossover_counter,section_p2.get(k));
          parent2.set(crossover_counter,section_p1.get(k));
          crossover_counter++;
        }
        
        float which_o = random(-1,1);//Randomly select whether modified parent 1 or modified parent 2 is added to offspring_pop
        if(which_o >= -1 && which_o <0){
          offspring_pop.add(parent1);
        }else if(which_o >= 0){
          offspring_pop.add(parent2);
        }
    ...
    if(which_scheme == 2){
      ...
      if(dice <= crossover_rate){//Crossover occurs
        ...
        ArrayList<Integer> section_p1 = new ArrayList<Integer>(parent1.subList(cp,cp+exchanged_section)); // A section of parent 1
        ArrayList<Integer> section_p2 = new ArrayList<Integer>(parent2.subList(cp,cp+exchanged_section)); // A section of parent 2
        
        e_genes = e_genes + section_p1.size();//Update the total number of genes that have been exchanged  
        //output.print(i + ":" + "\t" + cp + "\t" + section_p1.size());
        //output.print("\n");
        ...
        offspring_pop.add(parent1);//Add modified genotypes to "offspring_pop"
        offspring_pop.add(parent2);
      
        output.print(i + ":" + "\t" + cp + "\t" + section_p1.size());//Add data to .csv file. i = offspring id/ cp = crossover point / section_p1.size() = length of the exchanged section
        output.print("\n");
      }else if(dice > crossover_rate){//Crossover doesn't occur
        offspring_pop.add(parent1);//Add unmodified genotypes to "offspring_pop"
        offspring_pop.add(parent2);
      
        output.print(i + ":");//Add data to .csv file. i = offspring id
        output.print("\n");
      }
    }//Reproduction scheme 2 closes. 
